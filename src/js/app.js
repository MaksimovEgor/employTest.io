...
*/
const itemFirst = document.getElementById("item_1");
const itemSecond = document.getElementById("item_2");
const itemThird = document.getElementById("item_3");
const itemFourth = document.getElementById("item_4");

const lineA = new LeaderLine(itemFirst, itemSecond);
const lineB = new LeaderLine(itemSecond, itemThird);
const lineC = new LeaderLine(itemThird, itemFourth);
...
lineA.setOptions(options);
lineB.setOptions(options);
lineC.setOptions(options);

/*  leader-line-3-mask-bg-rect
 */
setTimeout(() => {
  document.getElementsByClassName("leader-line").style.display = "none";
  // document.getElementById('leader-line-3-mask-bg-rect').style.stroke = 'red'
}, 2000);
...
/* window.onload = function () {
  const circles = [...document.querySelectorAll(".icon-background")];
  let dot = document.createElement("span");
  dot.classList.add("dot");
  
    for (let i = 0; i < circles.length; i++) {
      circles[i].appendChild(dot.cloneNode(true));
    }
...
const arrow = document.querySelector('.custom-select-icons')
const mySelect = document.querySelector('.select-css')
...
function toggleList(whichWay) {
    ...
    arrow.classList.add('_opened')
    mySelect.classList.add('_opened')
	} else { // === 'Shut'
		csList.classList.add('hidden-all')
		csSelector.setAttribute('aria-expanded', 'false')
    arrow.classList.remove('_opened')
    mySelect.classList.remove('_opened')
	}
}
